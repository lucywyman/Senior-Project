...
def run(self):
	...
	s.close()
    \end{minted}
    \caption{Main loop of my testing processes. Recieve, process, return.}
\end{listing}
The dispenser thread works with the tester processes, checking for a pending
...
The select tree allows non-blocking port listening. If a new test is registered
on the socket bound to 'c', then it is registered in a FIFO queue, and a the
...
Figure 5 demonstrates a few interesting aspects of the CLI. The first is the
...
Figure 6 shows a test run of the \texttt{course} command with verbose on.
